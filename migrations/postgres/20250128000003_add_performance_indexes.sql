...
DO $$
BEGIN
    -- Check if compliance_alerts table exists before creating indexes
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'compliance_alerts') THEN
        -- Status-based compliance monitoring
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_alerts_status_created 
                 ON compliance_alerts (status, created_at DESC)';
        
        -- User-based compliance queries
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_alerts_user_created 
                 ON compliance_alerts (user_id, created_at DESC)';
        
        -- Severity-based alert filtering
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_alerts_severity_status 
                 ON compliance_alerts (severity, status, created_at DESC)';
        
        -- Assignment tracking
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_alerts_assigned_status 
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'aml_users') THEN
        ...
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_aml_users_risk_level_score 
                 ON aml_users (risk_level, risk_score DESC)';
        
        -- Geographic risk analysis
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_aml_users_country_risk 
                 ON aml_users (country_code, is_high_risk_country, risk_score DESC)';
        
        -- PEP and sanctions screening
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_aml_users_pep_sanctions 
                 ON aml_users (pep_status, sanction_status, risk_level)';
        
        -- Risk update tracking
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_aml_users_last_risk_update 
