...
type CreateTransactionRequest struct {
	...
	Type        store.TransactionType  `json:"type" validate:"required"`
...
type EntryRequest struct {
	...
	Type            store.EntryType `json:"type" validate:"required"`
...
func (e *endpoints) CreateTransaction(c echo.Context) error {
	...
	transaction := &store.Transaction{
		UserID:      store.NewUUID(req.UserID), // Assuming user_id is in context
		Type:        req.Type,
		Date:        store.NewUnixMilli(req.Date),
		Description: req.Description,
		Metadata:    req.Metadata,
	}

	// Convert entries
	txEntries := make([]store.Entry, len(req.Entries))
	for i, entry := range req.Entries {
		txEntries[i] = store.Entry{
			AccountID:       store.UUID{UUID: entry.AccountID},
	...
	transaction.Entries = store.NewEntries(txEntries)
