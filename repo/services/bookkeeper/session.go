...
import (
	...
	"github.com/litebittech/cex/common/set"
...
type Session struct {
	...
	pendingTransactions []store.Transaction
...
func (s *Session) setTransactionsToCommit(transactions []store.Transaction) {
...
func (s *Session) finalizeCheckpoint(ctx context.Context) (*cache.CommitState, error) {
	...
	userID := s.currentCheckpoint.UserID
	result, err := s.store.QueryTransactions(ctx, store.Filter{
		UserID:        userID,
...
// TODO: Cache
func (s *Session) checkAccountsExist(ctx context.Context, tx *store.Transaction) error {
	accountIDs := make(set.Set[uuid.UUID])
	for _, entry := range tx.Entries.Slice() {
		if !entry.IsSystemAccount {
			accountIDs.Include(entry.AccountID.UUID)
		}
	}

	accountIDsSlice := accountIDs.Slice()
	if len(accountIDsSlice) == 0 {
		return nil
	}

	accounts, err := s.store.GetUserAccounts(ctx, tx.UserID.UUID, accountIDsSlice)
	if err != nil {
		return errors.New("failed to check accounts").Wrap(err)
	}

	foundAccounts := make(set.Set[uuid.UUID])
	for _, account := range accounts {
		foundAccounts.Insert(account.ID.UUID)
	}

	for _, id := range accountIDsSlice {
		if _, exists := foundAccounts[id]; !exists {
			return errors.NotFound.Explain("account %s not found", id)
		}
	}
...
func (s *Session) CreateTransaction(ctx context.Context, tx *store.Transaction) error {
	if err := s.checkAccountsExist(ctx, tx); err != nil {
		return err
	}
	...
	s.setTransactionsToCommit([]store.Transaction{*tx})
	...
	if err := s.store.CreateTransaction(ctx, &s.pendingTransactions[0]); err != nil {
		return errors.New("failed to insert transaction in store").Wrap(err)
	}
...
func affectedBalanceKeys(transactions []store.Transaction) []types.BalanceKey {
