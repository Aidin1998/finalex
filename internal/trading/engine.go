...
func (e *OrderEngine) GetOrderBook(pair string) (map[string]interface{}, error) {
	...
	// Get the orderbook and then get a snapshot with depth 100
	orderBook := e.engine.GetOrderBook(pair)
	if orderBook == nil {
		return nil, fmt.Errorf("order book not found for pair: %s", pair)
	}

	// Get top 100 price levels
	bids, asks := orderBook.GetSnapshot(100)

	// Convert to a map for the API
